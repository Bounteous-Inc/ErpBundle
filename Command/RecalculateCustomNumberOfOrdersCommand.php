...
class RecalculateCustomNumberOfOrdersCommand extends ContainerAwareCommand implements CronCommandInterface
{
    const COMMAND_NAME = 'oro:cron:demacmedia:erp:number-of-orders:recalculate';
    const MAX_TIME_EXECUTION = 120; /* Maximum execution time in seconds. 120 = 2 minutes */
    const LOCK_FILENAME = 'number_of_orders_running.lock';
    protected $em;
    protected $x; // I know this is a terrible name but it's just the loop counter.
    protected $startedAt;
    ...
    public function configure()
    {
        $this
            ...
            ->setDescription('Perform re-calculation of number of orders for WebAccounts channel.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		for ($x = $this->getX(); $x <= $lastWebAccountId; $x = $x + $batchSize) {
            ...
            foreach ($iterableResult as $row) {
                ...
                ********************************************
                */
                $this->addFixNumberOfOrdersToJobQueue($deaEntity);
        ...
        // Finished execution!
        $output->writeln('Finished. Removing LOCK_FILENAME...');
...
/************* PRIVATE METHODS ***********************/
    ...
    private function addFixNumberOfOrdersToJobQueue(OroErpAccounts $entity)
    {
        ...
        // Creates a new entry at Queue so this command executes asynchronous.
        $job = new Job(
            'demacmedia:oro:erp:number-of-orders:update', 
